...
def train(withDropout):
    ...
    if withDropout:
        ...
        np.save(os.path.join(dir_path, 'other', 'npy', 'char_rnn_clipping_entropy_on_training_withDropout.npy'), np.array(entropy_on_training))
        np.save(os.path.join(dir_path, 'other', 'npy', 'char_rnn_clipping_accuracy_on_testing_withDropout.npy'), np.array(accuracy_on_testing))
        ...
        plt.savefig(os.path.join(dir_path, 'other', 'figure', 'char_rnn_clipping_withDropout.png'))  

    else:
        np.save(os.path.join(dir_path, 'other', 'npy', 'char_rnn_clipping_entropy_on_training_withoutDropout.npy'), np.array(entropy_on_training))
        np.save(os.path.join(dir_path, 'other', 'npy', 'char_rnn_clipping_accuracy_on_testing_withoutDropout.npy'), np.array(accuracy_on_testing))

        #plot
        plt.figure()
        plt.plot(entropy_on_training)
        plt.plot(accuracy_on_testing)
        plt.title('entropy / accuracy word')
        plt.xlabel('epoch')
        plt.legend(['entropy_on_training', 'accuracy_on_testing',], loc='upper left')
        plt.savefig(os.path.join(dir_path, 'other', 'figure', 'char_rnn_clipping_withoutDropout.png'))  
